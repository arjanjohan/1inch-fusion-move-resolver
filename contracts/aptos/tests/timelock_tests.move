...
module fusion_plus::timelock_tests {
    ...
    fun test_remaining_time() {
        ...
        assert!(timelock::get_remaining_time(&timelock) == FINALITY_DURATION - 30, 0);
        ...
        assert!(timelock::get_remaining_time(&timelock) == EXCLUSIVE_DURATION - 30, 0);

        // Check remaining time in private cancellation phase (30 seconds into cancellation)
        timestamp::update_global_time_for_test_secs(FINALITY_DURATION + EXCLUSIVE_DURATION + 30);
        assert!(timelock::get_remaining_time(&timelock) == CANCELLATION_DURATION - 30, 0);

        // Check remaining time in public cancellation phase
        timestamp::update_global_time_for_test_secs(FINALITY_DURATION + EXCLUSIVE_DURATION + CANCELLATION_DURATION + 1);
    ...
    fun test_total_duration_and_expiration() {
        ...
        let total_duration = FINALITY_DURATION + EXCLUSIVE_DURATION + CANCELLATION_DURATION;
        assert!(timelock::get_total_duration(&timelock) == total_duration, 0);

        let created_at = timelock::get_created_at(&timelock);
        assert!(timelock::get_expiration_time(&timelock) == created_at + total_duration, 0);
    }
    ...
    fun test_phase_constants() {
        ...
        // Move to private cancellation phase
        timestamp::fast_forward_seconds(EXCLUSIVE_DURATION + 1);
        assert!(timelock::get_phase(&timelock) == timelock::get_phase_private_cancellation(), 0);

        // Move to public cancellation phase
        timestamp::fast_forward_seconds(CANCELLATION_DURATION + 1);
        assert!(timelock::get_phase(&timelock) == timelock::get_phase_public_cancellation(), 0);
    }
    ...
    fun test_duration_validation_functions() {
        ...
        assert!(timelock::is_private_cancellation_duration_valid(CANCELLATION_DURATION), 0);
    ...
    fun test_create_timelock_zero_finality() {
        setup_test();
        timelock::new(0, EXCLUSIVE_DURATION, CANCELLATION_DURATION);
    }
    ...
    fun test_create_timelock_zero_exclusive() {
        setup_test();
        timelock::new(FINALITY_DURATION, 0, CANCELLATION_DURATION);
    }
    ...
    fun test_create_timelock_zero_cancellation() {
        setup_test();
        timelock::new(FINALITY_DURATION, EXCLUSIVE_DURATION, 0);
    }
    ...
    fun test_create_timelock_too_short_finality() {
        setup_test();
        timelock::new(30, EXCLUSIVE_DURATION, CANCELLATION_DURATION);
    }
    ...
    fun test_create_timelock_too_long_finality() {
        setup_test();
        timelock::new(40000000, EXCLUSIVE_DURATION, CANCELLATION_DURATION);
    }
    ...
    fun test_create_timelock_too_short_exclusive() {
        setup_test();
        timelock::new(FINALITY_DURATION, 30, CANCELLATION_DURATION);
    }
    ...
    fun test_create_timelock_too_long_exclusive() {
        setup_test();
        timelock::new(FINALITY_DURATION, 40000000, CANCELLATION_DURATION);
    }
    ...
    fun test_create_timelock_too_short_cancellation() {
        setup_test();
        timelock::new(FINALITY_DURATION, EXCLUSIVE_DURATION, 30);
    }
    ...
    fun test_create_timelock_too_long_cancellation() {
        setup_test();
        timelock::new(FINALITY_DURATION, EXCLUSIVE_DURATION, 40000000);
    }
}